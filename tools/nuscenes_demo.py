...
class DemoDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        if self.ext == '.bin':
            ...
            points = points.reshape(-1, 5)  # 'x', 'y', 'z', 'intensity', 'timestamp'
        elif self.ext == '.npy':
            points = np.load(self.sample_file_list[index])
            # print(f"self.src_feature_list: {self.point_feature_encoder.src_feature_list}")
            # print(f"points shape[-1]: {points.shape[-1]}")
            points = points[:, :5]  # 'x', 'y', 'z', 'intensity', 'elongation'
        else:
...
def draw_scenes(vis, points, gt_boxes=None, ref_boxes=None, ref_labels=None, ref_scores=None, point_colors=None,
                draw_origin=True):
